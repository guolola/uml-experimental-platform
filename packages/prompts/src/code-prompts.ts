// Owns code-generation prompt builders for Sandpack prototype planning, file operations, repair, and UI fidelity checks.
import type {
  CodeAppBlueprint,
  CodeBusinessLogic,
  CodeGenerationSpec,
...
const CODE_GENERATION_SEMANTICS = [
  ...
  "- 组件（构件）关系(component) -> 前端组件拆分、接口边界、可复用模块和依赖方向。",
  "- 部署设计(deployment) -> 前端环境提示和接口边界，可参考组件关系，不生成真实后端部署代码。",
].join("\n");
...
export function buildGenerateCodeUiBlueprintPrompt(
  ...
  return [
    ...
    "- 避免空壳营销页、单调卡片堆叠和软件工程实践平台默认工作台风格。",
...
export function buildGenerateCodeUiMockupPrompt(
  ...
  const prompt = [
    ...
    "- 避免空壳营销页、泛化仪表盘、单调卡片堆叠和软件工程实践平台默认工作台风格。",
...
export function buildGenerateCodeDesignTokensPrompt(
  ...
  return [
    ...
    "Token 必须服务于业务领域，不要复制软件工程实践平台默认工作台视觉。",
    "",
    "精简代码上下文：",
