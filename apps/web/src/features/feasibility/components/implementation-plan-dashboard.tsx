...
export function ImplementationPlanDashboard({ workspace, states, initialCandidateId, contextExists, generating, message, errorMessage, onRegenerate, onSave }: Props) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1024px] flex-col gap-6 px-4 py-6 sm:px-8 lg:px-12">
      ...
      </> : <section className="rounded-2xl border border-dashed bg-card p-8 text-center"><AlertTriangle className="mx-auto size-8 text-warning" /><h2 className="mt-3 text-lg">{t("feasibility.candidate.detailsMissingTitle", { name: activeCandidate?.name ?? t("feasibility.notProvided") })}</h2><p className="mt-2 text-sm text-muted-foreground">{t("feasibility.candidate.detailsMissingDescription")}</p><Button type="button" variant="outline" className="mt-4" onClick={() => void regenerate()} disabled={generating}><RefreshCw className={cn("size-4", generating && "animate-spin")} />{t("feasibility.regenerate")}</Button></section>}
