...
- ✅ **可行性分析与实施决策**
  从已接受规则生成系统上下文图（系统环境图）和可编辑实现方案，比较候选方案的架构、成本、周期、风险与约束，形成技术、运行、进度、经济和法律五类结论；需求规则、上下文或补充事实变化时会把旧产物标记为过期。
- ✅ **设计阶段建模**
...
- **可行性分析**
  生成带来源追踪的系统上下文图（系统环境图）以及候选实现方案，覆盖架构、数据、部署、安全、成本收益、里程碑、风险和五类可行性结论；报告生成前会检查上下文与实现方案是否仍然有效。
- **设计模型链路**
...
- `apps/api/src/normalizers/feasibility/`：系统上下文图（系统环境图）和实现方案输出修复。
