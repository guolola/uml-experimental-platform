...
| `PUBLIC_WEB_BASE_URL` | 是 | 前台唯一 HTTPS origin，例如 `https://jianglisoftware.com`；同时用于邮件链接、canonical、sitemap 和搜索引擎提交。生产 SEO 构建缺失时直接失败。 |
| `INDEXNOW_KEY` | 建议 | IndexNow 域名验证 key。发布脚本将其写入共享目录并通过 `/indexnow-key.txt` 提供，不得提交仓库。 |
| `BAIDU_PUSH_TOKEN` | 建议 | 百度搜索资源平台普通收录 API token。只在公开页面内容摘要变化时使用，日志不得输出原值。 |
